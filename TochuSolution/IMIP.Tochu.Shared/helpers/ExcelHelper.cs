...
namespace IMIP.Tochu.Shared.helpers
{
    ...
    {
        public static bool TryConvert(string input, Type targetType, out object? result)
        {
            result = null;
            ...
            try
            {
                ...
                if (type == typeof(int))
                {
                    if (int.TryParse(input, out var v)) { result = v; return true; }
                    return false;
                }
                ...
                {
                    if (DateTime.TryParse(input, out var v)) { result = v; return true; }
                    return false;
                }

                if (type.IsEnum)
                {
                    if (Enum.TryParse(type, input, true, out var v))
                    {
                        result = v;
                        return true;
                    }
            ...
            try
            {
                ...
                for (int i = 1; i < lines.Length; i++)
                {
                    ...
                    for (int j = 0; j < headers.Length; j++)
                    {
                        ...
                        var valueEach = values[j];
                        var prop = typeof(T).GetProperties().FirstOrDefault(p => Normalize(p.Name) == Normalize(header));
                        if (prop != null)
                        {
                            if (TryConvert(valueEach, prop.PropertyType, out var safeValue))
                            {
        ...
        public static List<T> ReadExcel<T>(string filePath) where T : new()
        {
            ...
            ExcelPackage.License.SetNonCommercialPersonal("IMIP.Tochu.Shared");
